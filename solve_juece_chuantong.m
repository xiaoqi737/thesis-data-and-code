...
%
% 使用示例见文件末尾。
function [lambda, omega_minus, omega_plus, rho, exitflag, z, fval] = solve_juece_chuantong(...
...
for r = 1:l
    idx = r;
    left = -omega_plus(r) - mu_mrp_p(r) + ( y_bar(idx,:) * lambda );
    rhs = norminv(p) * sigma_mrp_p(r) ;
    ceq(m + r) = rhs - left;
end
...
% eps_den = 1e-8;
% ceq(end) = (1/l) * sum( omega_plus ./ denom_den ) - (1 - eps_den);
