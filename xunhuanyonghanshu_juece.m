...
% 存储结果的结构体数组
results = struct();
label_zong = readmatrix("D:\内农大\学位论文\最终效率值.xlsx",'Sheet','Sheet1');
data_chanchu_juece_mu = readmatrix("D:\内农大\学位论文\期望值.xlsx",'Sheet','Sheet4');
data_touru_juece_mu = readmatrix("D:\内农大\学位论文\期望值.xlsx",'Sheet','Sheet2');
data_chanchu_juece_sig = readmatrix("D:\内农大\学位论文\方差值.xlsx",'Sheet','Sheet5');
data_touru_juece_sig = readmatrix("D:\内农大\学位论文\方差值.xlsx",'Sheet','Sheet3');
...
for i = 1:31
    ...
    if label == 0
        ...
        % 循环增加u_alpha直到无解
        while p > 0 && p <= max_p
            [lambda, omega_minus, omega_plus, rho, exitflag, z, fval] = solve_juece_chuantong(...
