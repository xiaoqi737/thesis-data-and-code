...
% 存储结果的结构体数组
results = struct();
label_zong = readmatrix("D:\内农大\学位论文\最终效率值.xlsx",'Sheet','Sheet3');
...
for i = 1:31
    ...
    m = length(mu_ip);
    l = length(mu_mrp_p);
    mu_bar = [data_touru_sample_mu(2:32,2:4),data_chanchu_sample_mu(2:32,5:6)]';
    sigma2_bar = [data_touru_sample_sig(2:32,2:4),data_chanchu_sample_sig(2:32,5:6)]';
    ...
    if label == 0
        ...
        % 循环增加u_alpha直到无解
        while p > 0 && p <= max_p
            [lambda, omega_minus, omega_plus, rho, exitflag, z, fval] = solve_rho_chuantong(...
