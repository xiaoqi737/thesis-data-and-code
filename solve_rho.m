...
function [lambda, omega_minus, omega_plus, rho, exitflag, z, fval] = solve_rho(...
...
for i = 1:m
    left = omega_minus(i) + mu_ip(i) - mu_bar(i,:) * lambda;
...
for r = 1:l
    idx = m + r;
    left = omega_plus(r) - mu_mrp_p(r) + d * ( mu_bar(idx,:) * lambda );
    rhs = norminv(p) * sqrt( sum( sigma2_bar(idx,:) .* (lambda'.^2) ) + sigma_mrp_p(r)^2 );
    c(m + r) = rhs - left;
end
for rr = 1:l
    denom_den = mu_mrp_p + norminv(p) * sqrt( sum( sigma2_bar(m+rr,:) .* (lambda'.^2) ) + sigma_mrp_p(rr)^2 );
