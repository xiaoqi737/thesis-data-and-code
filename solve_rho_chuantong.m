...
%
% 输入说明:
%   mu_bar        : (m + l) x nbar 矩阵， mū_{i,j}（前 m 行对应 i=1..m，后 l 行对应 m+r）
%   sigma2_bar    : (m + l) x nbar 矩阵， σ̄_{i,j}^2
...
function [lambda, s_minus, s_plus, rho, exitflag, z, fval] = solve_rho_chuantong(...
...
if any(size(sigma2_bar) ~= size(mu_bar))
...
s_minus0 = 0.01 + 0.1 * rand(m, 1);
s_plus0 = 0.01 + 0.1 * rand(l, 1);
x0 = [lambda0; s_minus0; s_plus0];
...
nonlcon = @(z) nlcons_rho(z, mu_bar, sigma2_bar, mu_ip, sigma_ip, mu_mrp_p, sigma_mrp_p, p, d, delta, m, l, nbar);
...
% 解析结果
lambda = z(1:nbar);
s_minus = z(nbar+1 : nbar + m);
s_plus  = z(nbar + m + 1 : end);
rho = obj(z);
...
lambda = z(1:nbar);
s_minus = z(nbar+1 : nbar + m);
s_plus  = z(nbar + m + 1 : nbar + m + l);

% 分母稳定性保证
eps_den = 1e-9;
num_term = 1 - (1/m) * sum( s_minus ./ denom_num );
den_term = 1 + (1/l) * sum( s_plus ./ denom_den );
if den_term <= eps_den
    r = 1e20 + (1/l)*sum(max(0, s_plus ./ denom_den));
    return;
end
r = num_term / den_term;
end
...
function [c, ceq] = nlcons_rho(z, mu_bar, sigma2_bar, mu_ip, sigma_ip, mu_mrp_p, sigma_mrp_p, p, d, delta, m, l, nbar)
lambda = z(1:nbar);
s_minus = z(nbar+1 : nbar + m);
s_plus  = z(nbar + m + 1 : nbar + m + l);
...
for i = 1:m
    left = -s_minus(i) + mu_ip(i) - mu_bar(i,:) * lambda;
    rhs = norminv(p) * sqrt( sum( sigma2_bar(i,:) .* (lambda'.^2) ) + sigma_ip(i)^2 );
    % left >= rhs  <=> rhs - left <= 0
    ceq(i) = rhs - left;
end
...
for r = 1:l
    idx = m + r;
    left = -s_plus(r) - mu_mrp_p(r) + d * ( mu_bar(idx,:) * lambda );
    rhs = norminv(p) * sqrt( sum( sigma2_bar(idx,:) .* (lambda'.^2) ) + sigma_mrp_p(r)^2 );
    ceq(m + r) = rhs - left;
end
...
% eps_den = 1e-8;
% ceq(end) = (1/l) * sum( s_plus ./ denom_den ) - (1 - eps_den);

% 等式约束： δ * sum(lambda) = δ  -> 当 δ~=0 等价于 sum(lambda)=1
if abs(delta) < 0.1
    ceq(end) = [];
else
    ceq(end) = sum(lambda) - 1;
end
end